...
export async function GET(
  ...
  try {
    ...
    if (!/^0x[0-9a-fA-F]{64}$/.test(requestId)) {
      throw new SubmitterProblem(400, "INVALID_REQUEST_ID", "requestId must be a 32-byte hex hash.");
    }
    const submission = await runtime.repository.getProjection(requestId.toLowerCase());
    if (!submission || submission.functionName !== "resolve_submission") {
      throw new SubmitterProblem(404, "SUBMISSION_NOT_FOUND", "Campaign submission not found.");
    }
