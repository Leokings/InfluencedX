...
test("exact Vercel caller claims and the independent service token are accepted", async () => {
  await verifyCallerToken(await token(), config, publicKey);
  requireServiceToken(config.serviceToken, config.serviceToken);
});

test("OIDC spoofing and every wrong workload binding fail closed", async () => {
  for (const patch of [
    { owner: "attacker" },
    ...
    await assert.rejects(
      ...
      (error: unknown) => (error as { code?: string }).code === "OPERATOR_AUTH_INVALID",
    );
  }
});

test("missing, malformed, and wrong service tokens fail independently", () => {
