...
export async function requireIngressAuth(request: Request, config: ReconcilerConfig): Promise<void> {
  await requireCallerOidc(request, config);
  requireServiceToken(request.headers.get(SERVICE_TOKEN_HEADER), config.serviceToken);
}

export async function requireCallerOidc(request: Request, config: ReconcilerConfig): Promise<void> {
...
function unauthorized(): never {
  throw new ReconcilerProblem(401, "RECONCILER_AUTH_INVALID", "Exact Vercel OIDC and the independent withdrawal service token are required.");
}
