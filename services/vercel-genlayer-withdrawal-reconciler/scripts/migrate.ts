...
const LEDGER_TABLE = "influencedx_withdrawal_reconciler_migrations";
const ADVISORY_LOCK_KEY = "3488961350311019282";
...
const names = (await readdir(directory))
  ...
  .sort();
if (names.length === 0) throw new Error("No withdrawal reconciler migrations were found.");
...
function assertAppliedPrefix(
  ...
  if (applied.length > migrations.length) {
    throw new Error("The withdrawal reconciler migration ledger references a missing migration file.");
  }
  for (let index = 0; index < applied.length; index += 1) {
    const migration = migrations[index];
    const record = applied[index];
    if (!migration || record.migration_name !== migration.name) {
      throw new Error("Applied withdrawal reconciler migrations are not an exact prefix of the migration files.");
    }
...
async function assertBootstrapSafe(client: pg.Client): Promise<void> {
  const presence = await client.query<{
    ...
    SELECT
      to_regclass(current_schema() || '.influencedx_withdrawal_reconciliations')::text AS status_table,
      to_regclass(current_schema() || '.influencedx_withdrawal_reconciliation_jobs')::text AS jobs_table,
      to_regclass(current_schema() || '.influencedx_withdrawal_reconciliation_signer_gate')::text AS gate_table
  `);
  const tables = Object.values(presence.rows[0] ?? {}).filter((value) => value !== null);
  if (tables.length === 0) return;
  if (tables.length !== 3) {
    throw new Error("Cannot bootstrap the withdrawal reconciler ledger over a partial service schema.");
  }
  ...
  const safety = await client.query<{
    ...
    SELECT
      EXISTS (SELECT 1 FROM influencedx_withdrawal_reconciliations) AS has_status_rows,
      EXISTS (SELECT 1 FROM influencedx_withdrawal_reconciliation_jobs) AS has_job_rows,
      EXISTS (
        SELECT 1
          FROM influencedx_withdrawal_reconciliation_signer_gate
         WHERE gate_name <> 'influencedx-withdrawal-confirmer-signer-v1'
  ...
  if (!row || row.has_status_rows || row.has_job_rows || row.has_unsafe_gate) {
    throw new Error(
      "Cannot bootstrap the withdrawal reconciler migration ledger unless its work tables are empty and signer gate is idle.",
    );
  }
}
