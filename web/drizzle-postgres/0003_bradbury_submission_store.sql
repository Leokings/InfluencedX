...
ON CONFLICT (gate_name) DO NOTHING;--> statement-breakpoint

CREATE INDEX IF NOT EXISTS xproof_bradbury_status_updated_idx
  ON xproof_bradbury_submission_status (updated_at DESC);--> statement-breakpoint
...
COMMENT ON TABLE xproof_bradbury_submission_status IS
  'Safe XProof Bradbury status projection. The web application may read this table by request_id.';--> statement-breakpoint
COMMENT ON TABLE xproof_bradbury_submission_jobs IS
  'Private submitter work records. envelope_json is purged as soon as broadcast completes or a terminal quarantine is recorded.';--> statement-breakpoint
