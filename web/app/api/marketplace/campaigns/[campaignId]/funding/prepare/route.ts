...
import { prepareGenLayerCampaignFunding } from "@/lib/marketplace-genlayer-service";
...
export async function POST(
  ...
  try {
    ...
    await enforceVerificationRateLimit(request, "marketplace-campaign-create", {
      subject: session.subject,
      wallet: session.wallet,
      requestId: campaignId,
    });
    const result = await prepareGenLayerCampaignFunding({
      campaignId,
      session,
      body,
    });
