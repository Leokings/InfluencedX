"""Guarded native-value canary for the isolated StudioNet V3 candidate.

This appends one one-atto cancelled campaign and one emitted withdrawal to the
candidate only. The production-facing V2 deployment is never selected here.

Run from the repository root with::

    $env:RUN_STUDIONET_V3_NATIVE_VALUE_CANARY = "1"
    gltest tests/integration/test_studionet_v3_native_value_canary.py -v -s --network studionet
"""
...
pytestmark = pytest.mark.skipif(
    os.getenv("RUN_STUDIONET_V3_NATIVE_VALUE_CANARY") != "1",
    reason="live isolated StudioNet V3 candidate mutation requires the canary guard",
)
...
def _assert_finalized_consensus_success(receipt: dict[str, Any]) -> None:
    ...
    assert len(leaders) == 1, receipt
    assert leaders[0].get("execution_result") == "SUCCESS", leaders[0]
    assert leaders[0].get("result", {}).get("status") == "return", leaders[0]
...
def test_v3_candidate_native_gen_escrow_refund_and_eoa_withdrawal(
    gl_client: Any, default_account: Any
) -> None:
    """Prove one atto enters V3 escrow, becomes credit, and reaches the EOA."""
    ...
    ).build_contract(MARKETPLACE, account=brand)
    ...
    assert config["resolution_recovery_delay_seconds"] == 15 * 60
    assert config["paused"] is False

    now = int(time.time())
    client_nonce = "v3-canary-" + secrets.token_hex(12)
    args_without_id = [
        client_nonce,
        "X",
        "StudioNet V3 canary",
        "One-atto V3 native escrow and withdrawal integration canary.",
        json.dumps(["InfluencedX"], separators=(",", ":")),
        "[]",
        True,
        now + 3_600,
        now + 7_200,
    ...
    campaign_id = marketplace.compute_campaign_id(
        args=[brand_calldata, *args_without_id]
    ).call()
    counts_before = marketplace.get_counts(args=[]).call()
    brand_balance_before = gl_client.get_balance(brand_address)
    if brand_balance_before < CANARY_AMOUNT_ATTO:
        faucet = gl_client.provider.make_request(
            "sim_fundAccount",
            [brand_address, CANARY_AMOUNT_ATTO - brand_balance_before],
        )
        assert "error" not in faucet, faucet
    ...
    assert str(transfer_receipt.get("triggered_on", "")).upper() == "FINALIZED"
    assert str(
